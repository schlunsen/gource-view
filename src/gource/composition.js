 ...
 */
export function createComposition({ ctx, data, config, renderer, W, H, palette }) {
  ...
  function background() {
    ...
    bg.addColorStop(0, P.bgInner); bg.addColorStop(1, P.bgOuter)
  ...
  function chrome(progress) {
    ...
    ctx.fillStyle = P.accent; ctx.font = `16px ${FONT_MONO}`
    ctx.fillText('GOURCE VIEW / REPOSITORY HISTORY', m, 48)
    ctx.fillStyle = P.ink; ctx.font = `500 ${portrait ? 28 : 32}px ${FONT_SANS}`
    ctx.fillText(config.title || repoLabel(), m, 94, titleMax)
    ctx.font = `16px ${FONT_MONO}`; ctx.fillStyle = P.inkSoft
    ctx.fillText(`${repoLabel()}  ·  ${data.stats.commits} commits  ·  ${data.stats.authors} authors`, m, H - 50, W - 2 * m)
    ctx.fillStyle = P.rule; ctx.fillRect(m, H - 26, W - 2 * m, 3)
    ctx.fillStyle = P.accent; ctx.fillRect(m, H - 26, (W - 2 * m) * progress, 3)
  }
  ...
  function introCardAt(p, W, H) {
    ...
    ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic'
    ctx.fillStyle = P.accent; ctx.fillRect(x - 40, base - 110 + rise, 6, 150 * easeOut(p / 0.5))
    ctx.fillStyle = P.accent; ctx.font = `600 18px ${FONT_MONO}`
    ...
    ctx.fillStyle = P.ink; ctx.font = `700 ${big}px ${FONT_SANS}`
    ctx.fillText(config.title || repoLabel(), x, base + rise, W - x - 60)
    ctx.fillStyle = P.inkSoft; ctx.font = `${small}px ${FONT_MONO}`
  ...
  function outroCardAt(p, W, H) {
    ...
    try { ctx.letterSpacing = '0px' } catch { /* older canvas */ }
    ctx.fillStyle = P.ink; ctx.font = `700 40px ${FONT_SANS}`
