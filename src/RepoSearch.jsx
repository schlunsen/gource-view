 ...
 */
export default function RepoSearch({ onPick, placeholder = 'Search GitHub, or owner/repo…', disabled = false, label = 'Search for a project' }) {
  ...
  useEffect(() => {
    // An exact owner/repo is a destination, not a search.
    if (term.length < 2 || /^[\w.-]+\/[\w.-]+$/.test(term)) { setResults([]); setError(''); return }
    let cancelled = false
  ...
  const choose = name => { setQuery(''); setResults([]); setActive(-1); onPick(name) }
  const submit = e => {
    e.preventDefault()
  ...
  return (
    <form className="repo-search" ref={box} onSubmit={submit} role="search">
      <input
        ref={input} value={query} disabled={disabled} aria-label={label} placeholder={placeholder}
      ...
      <button type="submit" disabled={disabled || !term}>{busy ? '…' : 'Add'}</button>
