...
cd "$TMP/pub"
if git grep -n -i -E "moon\.nzero|nzero\.pro|lunar-?rails|clovr_pat_|clovrlabs|padronsec|privateprompt|23\.88\.55\.82|46\.224\.72\.238|78\.46\.219\.157|167\.233\.208\.238" -- . ':!server/music/*' | grep -v "^scripts/sync-public.sh" ; then echo "refusing to publish: platform references above" >&2; exit 1; fi
git add -A
