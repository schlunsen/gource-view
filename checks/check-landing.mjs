...
try {
  ...
  assert.equal(Number(await timeline.inputValue()), paused, 'pause freezes shared time')
  await timeline.fill('1')
  await timeline.dispatchEvent('input')
  await page.getByRole('button', { name: 'Replay week', exact: true }).click()
  assert.ok(Number(await timeline.inputValue()) < .02, 'replay returns to the start of the week')
  await page.getByRole('button', { name: 'Pause', exact: true }).click()
  ...
  console.log('Landing: ranking, four animated previews, shared playback, pause and replay, responsive grid, viewer navigation and legacy links passed.')
